# 2026-09-09 剩餘列印包：右腿完成之後

此包是本站當次建造進度的續印範例，不是全機完整套件。31 件：30 PLA、1 TPU。每個 STL 印一次，兩件一體框架已各存一份。

前提：右腿及 trunk_bottom 已完成；庫存已有 roll_motor_bottom 1 件、roll_motor_top 2 件，左腿各取一件，剩一件 top 備用。身體與頭部尚未印。若你的進度不同，請依上游完整列印指南重新點收，不能直接把本包當成全部用量。
...
改版與重建：https://microduck.intemotech.com/articles/one-piece-shin
授權：附 LICENSE-Open-Duck-Mini.txt；一體件為本站衍生修改。
